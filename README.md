...
> **Continuing on another computer?** Use **`prototype/three-stop-tour`**, not the older Bay checkpoint. The owner authorized this branch checkpoint/push, including the connected Tour, readability/glass HUD and destination compass. See the **[current cross-computer handoff](docs/iteration-plan.md#current-checkpoint-and-cross-computer-handoff--2026-09-08)** for clone/update commands, revision identification with `git log`, verification and host limitations. The primary verifies Git separately; this is not a claim that the push already succeeded. Production remains original-only; no merge or Pages deployment is authorized.

Requires Node.js 20.19+ or 22.12+ (recommend Node.js 24; verified with v24.18.0 and npm 11.16.0).
...
Open **http://127.0.0.1:5173/?prototype=tour** for the connected **Sunrise Bakery → Stargaze Station → Windmill Garden** journey. Implementation and primary independent checks are complete on `prototype/three-stop-tour`, based on `412ef66`; see the current verification limitation below. The owner authorized this branch checkpoint/push for cross-computer continuation, with Git execution/verification handled separately by the primary. Owner hands-on approval of the connected journey and destination compass remains pending. No production promotion, merge or Pages deployment is authorized; the public game remains original-only.
...
**Primary-supplied verification (not run by the documentation-only worker):** **192 unit tests in 21 files**, TypeScript/production build and **6/6 production-isolation cases** passed. Only the existing >500kB build warning remains. **All 85 distinct development cases have passing observations across sequential runs (42 + 1 + 42), NOT one clean 85/85 all-in-one pass.** The first 42 included all 23 current navigation cases, original/standalone routes and desktop wide/short real-input Tours/closing road. The full command was stopped after the existing 320px Tour case timed out in initial `page.goto`, before gameplay/layout assertions; the identical case then passed **3/3** in fresh browsers without code or timeout changes. The separate UI-readability suite passed **42/42** after a webServer startup timeout was handled by checking port ownership and starting Vite. Production checks ran afterward. No product regression or permanent environment fix is claimed.
...
Run development-browser and production-preview checks **sequentially**. If this Windows host's long-lived Edge session stalls page loads, run files/suites in separate fresh processes; see the [handoff's reproduction commands](docs/iteration-plan.md#reproduce-checks-and-host-limitations), including the 320px Tour repeat and separate UI-readability suite. Check port ownership before handling server startup issues.
