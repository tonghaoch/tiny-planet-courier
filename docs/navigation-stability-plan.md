...
**Current presentation follow-up — 2026-09-09 (Windows local date):** the left hint/context stack, left toast and `.18` lightly frosted navigation are implemented, independently verified and **owner-visually accepted/frozen as the checkpoint baseline**. Source scope is `src/ui.ts` + `src/style.css`, with permanent regressions only in `tests/browser/ui-readability.spec.ts`; destination bearings and gameplay are unchanged. See the [current handoff](iteration-plan.md#current-left-context-and-soft-glass-iteration--2026-09-09) for geometry, contrast limits, the confirmed reduced-motion transition repair, and **192 unit / 89 distinct grouped development / final 6 production** passing results. The macOS refinement details below are historical, not the current styling or scope.
...
**Checkpoint and acceptance boundary — 2026-09-09:** **`aa853e6`, `Compact navigation HUD and fix mobile welcome layout`**, is the previous published baseline after `56c92fc`; its earlier visual approval and completed push are distinct from the owner's newly confirmed CURRENT-version praise ("very good, great, this version is very good") and explicit **commit + push, then the next step** request. This owner-approved checkpoint packages the verified left-hint/left-toast/light-frost implementation. Use the [cross-computer handoff](iteration-plan.md#current-checkpoint-and-cross-computer-handoff--2026-09-08) and `git log -1 --oneline` after clone/switch/pull to identify the actual received revision. The primary owns Git execution and remote-SHA verification; no new push success or SHA is claimed here. Authorization covers this prototype-branch checkpoint/push only, not prototype promotion, main merge or Pages deployment. Development-browser and production-preview checks must run sequentially.

**Next gate:** keep the accepted HUD visual baseline frozen and have the owner drive **Bakery → Station → Garden → closing road**, including at least a recovery/restart check. Assess destination-compass usability, road handoffs, left-side hints during actual driving and whole-journey pacing; neither visual praise nor automated tests satisfies these gameplay gates. Do not default to more cosmetics or features. If gameplay is accepted, production promotion/main merge/Pages can be proposed separately, still requiring explicit authorization.
