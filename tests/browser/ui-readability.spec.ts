...
async function drivingVisibility(page: Page) {
  return page.evaluate(() => {
    ...
    return {
      ...state,
      overlays: ['#navigation-hud', '#mission-hint', '.mission-card', '.toast.visible'].flatMap(selector => {
...
function expectVanClear(state: Awaited<ReturnType<typeof drivingVisibility>>, width: number, height: number) {
  ...
  for (const overlay of state.overlays) {
    expect(
      intersects(overlay, { x: van.left, y: van.top, width: van.right - van.left, height: van.bottom - van.top }),
      `${overlay.selector} covers van in ${state.phase ?? 'standard'}: ${JSON.stringify({ overlay, van })}`,
    ).toBe(false);
  }
}
  ...
  await expect
    .poll(
      async () => {
        ...
        return Math.max(
          Math.abs(geometry.actual - geometry.cached),
          Math.abs(geometry.navigationTop - geometry.cachedTop),
          Math.abs(geometry.navigationBottom - geometry.cachedBottom),
          Math.abs(geometry.navigationLeft - geometry.cachedLeft),
        );
      },
      { message: 'Cached context must include the currently styled, visible hint and parcel' },
    )
...
async function expectToastLayout(page: Page) {
  ...
  expect(Math.abs(toast.x - (viewport.width <= 760 || landscape ? 16 : viewport.width * 0.043))).toBeLessThanOrEqual(1);
  expect(toast.width).toBeLessThanOrEqual(
    landscape ? Math.min(260, viewport.width * 0.3) : viewport.width <= 760 ? 220 : 224,
  );
  ...
  if (active && !landscape) {
    ...
    if (viewport.width > 760)
      expect(toast.x + toast.width).toBeLessThanOrEqual((await box(page, '#navigation-hud')).x - 12);
  }
  ...
  for (const selector of [
    ...
    if (await page.locator(selector).isVisible())
      expect(intersects(toast, await box(page, selector)), `toast vs ${selector}`).toBe(false);
  }
}
...
async function expectNavigationSurface(page: Page, expected?: NavigationSurface) {
  const surface = await page.locator('#navigation-hud').evaluate(hud => {
    ...
    const content = [
      ...
      const ratios = [255, 0].map(channel => {
        ...
        return {
          direct: contrast(composite(color, background), background),
          outlined: contrast(composite(color, protection), protection),
        };
      });
      return {
        selector,
        ...
        alpha: color.alpha,
        outline,
        fillOpacity: arrow ? Number(paint.fillOpacity) : 1,
        strokeOpacity: arrow ? Number(paint.strokeOpacity) : 1,
        strokeWidth: parseFloat(paint.getPropertyValue(arrow ? 'stroke-width' : '-webkit-text-stroke-width')),
        paintOrder: paint.getPropertyValue('paint-order'),
        vectorEffect: paint.getPropertyValue('vector-effect'),
        backings: layers
          .filter(layer => layer !== hud && !layer.classList.contains('direction-disc'))
          .map(layer => ({
            color: rgba(getComputedStyle(layer).backgroundColor),
            image: getComputedStyle(layer).backgroundImage,
          })),
        whiteContrast: ratios[0],
        darkContrast: ratios[1],
      };
    });
  ...
  expect(surface.disc).toEqual({ rgb: [41, 75, 77], alpha: 0.22 });
  if (treatment === 'glass') {
    expect(surface.supportsFilter).toBe(true);
    expect(surface.reduced).toBe(false);
    expect(surface.background.alpha).toBe(0.18);
    ...
    expect(surface.background.alpha).toBeGreaterThanOrEqual(0.95);
  ...
  for (const sample of surface.content) {
    ...
    expect(sample.rgb, `${sample.selector} foreground color`).toEqual(
      sample.selector.includes('arrow') ? [255, 197, 156] : [255, 253, 245],
    );
    ...
    for (const [scene, ratios] of [
      ['white', sample.whiteContrast],
      ['dark', sample.darkContrast],
    ] as const) {
      expect(ratios.outlined, `${sample.selector} protective outline over ${scene}`).toBeGreaterThanOrEqual(minimum);
      if (treatment !== 'glass' && !sample.detached)
        expect(ratios.direct, `${sample.selector} opaque fallback over ${scene}`).toBeGreaterThanOrEqual(minimum);
    }
  }
  return surface;
}
...
async function expectNavigationLayout(page: Page, width: number, treatment?: NavigationSurface) {
  ...
  expect(
    await page.locator('#direction-arrow').evaluate(element => parseFloat(getComputedStyle(element).width)),
  ).toBeGreaterThanOrEqual(width <= 760 ? 28 : 32);
  ...
  for (const selector of ['.navigation-label', '#mission-index'])
    expect(await font(page, selector)).toBeGreaterThanOrEqual(10);
  const geometries = await page.locator('#navigation-hud, #mission-hint').evaluateAll(elements =>
    elements.map(element => {
      ...
      for (let current: Element | null = element; current; current = current.parentElement) {
        ...
        if (
          ...
          scaledAncestors.push(current.id || current.tagName);
      }
  ...
  expect(await page.locator('#navigation-hud').evaluate(element => getComputedStyle(element).pointerEvents)).toBe(
    'none',
  );
  await expectContextGeometry(page);
  ...
  expect(await page.locator('#mission-context').evaluate(element => getComputedStyle(element).pointerEvents)).toBe(
    'none',
  );
  ...
  if (width <= 760) {
    ...
    expect(Math.abs(hint.x - width * 0.043)).toBeLessThanOrEqual(1);
  ...
  const overflow = await page
    .locator('#navigation-hud, #mission-name, #mission-hint, .navigation-distance')
    .evaluateAll(elements =>
      elements.filter(element => element.scrollWidth > element.clientWidth + 1).map(element => element.id),
    );
...
for (const treatment of ['glass', 'fallback', 'reduced'] as const) {
  test(`navigation glass treatment: ${treatment} preserves content opacity and contrast`, async ({
    page,
    browserName,
  }) => {
    ...
    await cdp.send('Emulation.setEmulatedMedia', {
      features: [{ name: 'prefers-reduced-transparency', value: treatment === 'reduced' ? 'reduce' : 'no-preference' }],
    });
    ...
    if (treatment === 'fallback') {
      ...
      await page.route('**/src/style.css*', async route => {
        ...
        await route.fulfill({
          response,
          body: css.replace(gate, '@supports (backdrop-filter: unsupported-test-value)'),
        });
      });
    }
    ...
    if (treatment === 'reduced') {
      test.skip(
        ...
        'Browser does not expose reduced-transparency emulation.',
      );
    }
    ...
    await test
      .info()
      .attach(`navigation-${treatment}`, { body: JSON.stringify(surface, null, 2), contentType: 'application/json' });
    ...
    for (const background of ['rgb(255, 255, 255)', 'rgb(0, 0, 0)']) {
      ...
      expect(await page.locator('#stage').evaluate(element => getComputedStyle(element).backgroundColor)).toBe(
        background,
      );
      await expectNavigationSurface(page, treatment);
  ...
  for (const mode of modes) {
    test(`readable welcome and centered HUD in ${mode} at ${viewport.width}x${viewport.height}`, async ({ page }) => {
      ...
      if (viewport.width <= 390) {
        const frame = (await planetSnapshot(page)).welcomeFrame;
      ...
      for (
        let tries = 0;
        tries < 8 && (await start.boundingBox())!.y + (await start.boundingBox())!.height > viewport.height;
        tries++
      ) {
      ...
      expect(overflow).toEqual([]);
      if (await page.locator('.home-footer').isVisible())
        expect(intersects(await box(page, '.hero-copy'), await box(page, '.home-footer'))).toBe(false);
      ...
      if (viewport.width === 844) {
        ...
        expect(state.overlays).toEqual(
          expect.arrayContaining([expect.objectContaining({ selector: '.toast.visible' })]),
        );
      ...
      const baseline =
        mode === 'tour' &&
        initialTourBounds.find(
          sample => sample.viewport.width === viewport.width && sample.viewport.height === viewport.height,
        );
      if (baseline) {
        ...
        expect(
          rendered.height,
          'Removing the hint row makes the initial navigation strictly shorter',
        ).toBeLessThanOrEqual(baseline.height - baseline.removed);
...
test('live Tour target updates, recovery, restart, completion and home hide stale navigation', async ({ page }) => {
  ...
  for (const [index, name] of ['Sunrise Bakery', 'Stargaze Station', 'Windmill Garden'].entries()) {
    ...
    await expect(page.locator('#mission-name')).toHaveText(name);
    if (index > 0)
      await expect(page.locator('#mission-hint')).toContainText(/Reverse and turn|connecting road|deliveries are safe/);
    // State fixture only, not route-playability evidence.
    await dockAtTarget(page);
    await expect.poll(async () => (await planetSnapshot(page)).index).toBe(index + 1);
  }
  await expectNavigationVisibility(page, false);
...
for (const [viewport, reducedMotion] of [
  [viewports[0], 'no-preference'],
  [viewports[0], 'reduce'],
  [viewports[4], 'no-preference'],
  [viewports[4], 'reduce'],
] as const) {
  test(`initial and handoff Tour toast stays left with a real context gap at ${viewport.width}x${viewport.height} (${reducedMotion})`, async ({
    browser,
  }) => {
  ...
  try {
    ...
    for (let i = 0; i < selectors.length; i++) {
      for (let j = i + 1; j < selectors.length; j++) {
        expect(
          intersects(await box(page, selectors[i]), await box(page, selectors[j])),
          `${selectors[i]} vs ${selectors[j]}`,
        ).toBe(false);
      }
    }
  ...
  test(`short-landscape van stays clear during a real Bay boost launch (touch=${hasTouch})`, async ({ browser }) => {
    ...
    try {
      ...
      while (Date.now() < deadline) {
        ...
        if (state.overlays.some((overlay: { selector: string }) => overlay.selector === '.toast.visible'))
          initialToastSamples++;
  ...
  test(`long-copy presentation, label avoidance and reduced motion at ${viewport.width}x${viewport.height}`, async ({
    ...
    await page.addInitScript(() => {
      ...
      window.addEventListener('error', event => {
        if (event.message.includes('ResizeObserver')) window.__geometryErrors!.push(event.message);
      });
    });
    await page.route('**/ui-fixture', route =>
      route.fulfill({
        contentType: 'text/html',
        body: '<html lang="en"><head><link rel="stylesheet" href="/src/style.css"></head><body><div id="app"></div></body></html>',
      }),
    );
    await page.goto('/ui-fixture');
    await page.evaluate(async () => {
      const uiPath = '/src/ui.ts',
        gamePath = '/src/game.ts',
        prototypePath = '/src/delivery-prototypes.ts';
      const { UI } = (await import(uiPath)) as typeof import('../../src/ui'),
        { DeliveryRun } = (await import(gamePath)) as typeof import('../../src/game'),
        { PROTOTYPES } = (await import(prototypePath)) as typeof import('../../src/delivery-prototypes');
      const mathPath = '/src/math.ts';
      const { spherical } = (await import(mathPath)) as typeof import('../../src/math');
      const ui = new UI(PROTOTYPES.tour);
      const destinations = [
        {
          id: 'long-name',
          name: 'Stargaze Station Observatory',
      ...
      run.start();
      ui.update(run, 0, 1, 12, Math.PI - 0.02, {
        phase: 'grounded',
        onRamp: false,
        onCoastalRoad: false,
    ...
    await page.evaluate(() => {
      const { ui, run } = window.__presentation!;
      ui.resetJourney();
      ui.update(run, 0, 1, 12, -Math.PI + 0.03, {
        phase: 'grounded',
        onRamp: false,
        onCoastalRoad: false,
        nearDestination: false,
    ...
    await expect(page.locator('#mission-hint')).toHaveText(
      'Follow the connecting road to Stargaze Station Observatory.',
    );
    ...
    expect(Math.abs(Math.atan2(Math.sin(rendered - before), Math.cos(rendered - before)) - 0.05)).toBeLessThan(0.00002);
    expect(Math.abs(rendered - (-Math.PI + 0.03))).toBeLessThan(0.00001);
    expect(await page.evaluate(() => window.__presentation!.ui.getNavigationPresentation().displayedHeading)).toBe(
      -Math.PI + 0.03,
    );
    expect(
      await page.locator('#direction-arrow').evaluate(element => getComputedStyle(element).transitionProperty),
    ).toBe('none');
    await page.waitForTimeout(100);
    await expectNavigationLayout(page, viewport.width);
    expect(await box(page, '#navigation-hud'), 'Hint wording never increases the central panel').toEqual(
      longNavigation,
    );
    const geometry = await page.evaluate(() => {
      ...
      try {
        ...
        const clear = ui.canShowTargetLabel(innerWidth / 2, innerHeight * 0.68);
        for (let i = 0; i < 100; i++) {
          ui.getWelcomeFrame();
          ui.canShowTargetLabel(innerWidth / 2, innerHeight / 2);
        }
    ...
    expect(await page.evaluate(() => window.__geometryErrors)).toEqual([]);
    for (const resized of [viewport.width <= 760 ? viewports[0] : viewports[4], viewport]) {
      ...
      for (const reducedMotion of ['no-preference', 'reduce'] as const) {
        ...
        await page.evaluate(() =>
          window.__presentation!.ui.toast('Follow the connecting road to Stargaze Station Observatory.'),
        );
        await expectToastLayout(page);
    ...
    if (viewport.width <= 760 || viewport.height > 500) {
      ...
      await expect
        .poll(
          async () =>
            (await box(page, '#toast')).y -
            (await box(page, '#tour-result')).y -
            (await box(page, '#tour-result')).height,
        )
    ...
    await page.evaluate(() => {
      ...
      run.start();
      ui.update(run, 0, 1, 0, 0);
      ui.showError('Graphics connection lost. Reload to retry.');
    });
    await expectNavigationVisibility(page, false);
