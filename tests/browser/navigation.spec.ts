import type { PlanetTestSnapshot } from '../../src/dev/test-bridge-types';
import { setPlanetControls, dockAtTarget, planetRoutes, setNavigationFixture } from '../helpers/planet-test';
...
function expectDestinationBearing(state: PlanetTestSnapshot, destination: number[] = state.target!) {
  ...
  test(`real UI angle fixture: interrupted rendered retarget (reduced motion=${reduced})`, async ({ page }) => {
    ...
    await page.route('**/navigation-fixture', route =>
      route.fulfill({
        ...
        body: '<link rel="stylesheet" href="/src/style.css"><div id="app"></div>',
      }),
    );
    ...
    const samples = await page.evaluate(async reduced => {
      ...
      await new Promise<void>(resolve => {
        const frame = (now: number) => {
          ...
          if (now - start > 330 && values.length >= 8) resolve();
      ...
      command(120);
      ui.setMode('paused');
      const paused = rendered();
      ui.advanceNavigation(0.2);
    ...
    expect(Math.abs(angleDelta(samples.final, (-20 * Math.PI) / 180))).toBeLessThan(0.025);
...
for (const mode of ['bay', 'station', 'garden', 'tour', 'standard']) {
  test(`navigation state fixtures: ${mode} pad crossing, hysteresis, passive snapshots and reset`, async ({ page }) => {
    ...
    const states = await page.evaluate(radius => {
      ...
      if (!bridge) throw new Error('Planet test bridge missing; load a DEV page with ?test=1.');
      const result = [];
      for (const offset of [-0.02, 0, 0.02, radius + 0.1, radius + 0.26, radius + 0.1, 0.02]) {
        bridge.setNavigationFixture({ targetOffset: offset });
        result.push({
          ...bridge.snapshot().guidance,
    ...
    expect(states.result.map(s => s.mode)).toEqual([
      'parking',
      'parking',
      'parking',
      'parking',
    ...
    expect(states.repeated.every(s => JSON.stringify(s.guidance) === JSON.stringify(states.before.guidance))).toBe(
      true,
    );
    ...
    await setNavigationFixture(page, { targetOffset: DELIVERY_RADIUS + 0.1 });
...
for (const mode of ['bay', 'tour', 'standard']) {
  test(`slow physical pad-center crossing keeps parking and unchanged handoff in ${mode}`, async ({ page }) => {
    ...
    await setNavigationFixture(page, { targetOffset: -0.1, speed: 3, reset: true });
    await expect(page.locator('#mission-hint')).toHaveText('Brake to make your delivery.');
    expect((await snapshot(page)).parkedFor).toBe(0);
    const before = await page.evaluate(() => {
      const bridge = window.__planetTest;
      if (!bridge) throw new Error('Planet test bridge missing; load a DEV page with ?test=1.');
      bridge.setNavigationFixture({ targetOffset: -0.04, speed: 0.7, reset: true });
...
test('legacy airborne presentation is truthful without altering altitude eligibility', async ({ page }) => {
  ...
  const { state, coincident } = await page.evaluate(() => {
    const bridge = window.__planetTest;
    if (!bridge) throw new Error('Planet test bridge missing; load a DEV page with ?test=1.');
    bridge.setNavigationFixture({ targetOffset: 0.1, altitude: 0.1 });
    const state = bridge.snapshot();
    bridge.setNavigationFixture({ targetOffset: 0, altitude: 0.1 });
    return {
      state,
      coincident: {
        ...bridge.snapshot(),
    ...
    const values = await page.evaluate(mode => {
      ...
      const coordinates =
        mode === 'garden'
          ...
          : [
              { x: -6, y: 1.299, heading: Math.PI / 2 },
              { x: -6, y: 1.301, heading: Math.PI / 2 },
            ];
      return coordinates.map((coordinate, index) => {
        bridge.setNavigationFixture({ ...coordinate, reset: index === 0 });
        return bridge.snapshot();
      });
    }, mode);
    expect(
      (Math.abs(angleDelta(values[0].guidance!.canonicalHeading!, values[1].guidance!.canonicalHeading!)) * 180) /
        Math.PI,
    ).toBeLessThan(0.2);
      ...
      for (const heading of [0, Math.PI / 2, Math.PI]) {
        const state = await page.evaluate(
          pose => {
            ...
            return {
              ...bridge.snapshot(),
              name: document.querySelector('#mission-name')!.textContent,
        ...
        if (index < bends.length) {
          ...
          if (mode === 'bay' || mode === 'tour') {
            ...
            expect(level.sampleSurface(normal.clone().lerp(level.destination.normal, 0.5).normalize()).kind).toBe(
              'water',
            );
          }
...
test('original-game compass tracks driving, turning and destination handoffs', async ({ page }) => {
  ...
  expect(surfaceDistance(vector(before.normal), vector(after.normal))).toBeGreaterThan(0.1);
  expect(Math.abs(angleDelta(headings[0]!, headings.at(-1)!))).toBeGreaterThan(0.1);
...
for (const mode of ['bay', 'station', 'garden', 'tour']) {
  test(`real route-fixture controller drives ${mode} while the compass points to each delivery`, async ({ page }) => {
    ...
    const legs =
      'legs' in routes
        ? routes.legs.map(leg => ({ ...leg, path: leg.wide }))
        : [{ index: 0, destination: routes.destination, path: 'safe' in routes ? routes.safe : routes.outer }];
