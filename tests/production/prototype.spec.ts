...
async function expectOfficialWelcome(page: Page) {
  ...
  await expect(page.locator('.hero-description')).toHaveText(/Bakery, station, garden\. One connected journey\.\s*Pick your paths and follow the roads between stops\./);
...
for (const query of [
  '', '?test=1', '?prototype=standard&test=1',
  '?prototype=bay&test=1', '?prototype=station&test=1', '?prototype=garden&test=1', '?prototype=tour&test=1',
  '?prototype=&test=1', '?prototype=unknown&test=1', '?prototype=constructor&test=1', '?prototype=__proto__&test=1',
  '?prototype=standard&prototype=bay&test=1', '?prototype=unknown&prototype=standard&test=1',
]) {
...
test('public Tour supports keyboard driving, pause, recovery, restart and a clean home/start', async ({ page }) => {
  ...
  await page.keyboard.down('KeyW');
  try { await expect.poll(() => speed(page)).toBeGreaterThan(5); }
  finally { await page.keyboard.up('KeyW'); }
...
test('narrow public welcome fits and touch UI drives with real pointer input', async ({ page }) => {
  ...
  const overflow = await page.locator('.home-view, .hero-copy, .hero-description, .start-caption').evaluateAll(elements =>
