...
function flowingPath(nodes: readonly LevelPoint[]): LevelPoint[] {
  const curve = new CatmullRomCurve3(nodes.map(p => new Vector3(p.x, p.y, 0)), false, 'centripetal');
...
const outer = flowingPath([spawn, fork, point(-9.7, -2.8), point(-6.1, -4.6), point(-2.8, -5.1), point(0.5, -4.1), point(2.2, -2.1), point(1.8, -0.4), pad.center]);
const inner = flowingPath([spawn, fork, point(-8.3, 1.65), point(-6.4, 1.95), point(-4.7, 0.1), point(-3.1, -1.55), point(-1.45, -1.35), pad.center]);
const footprint = [point(-15, -4), point(-12, -7.2), point(-1, -7.2), point(4.5, -4), point(4.5, 3.5), point(1.5, 5.5), point(-10.5, 5.5), point(-15, 2.5)];
...
export const GARDEN_LEVEL = {
  ...
  arrows: [
    arrowAt(outer, 0.23, 'outer'), arrowAt(outer, 0.47, 'outer'), arrowAt(outer, 0.72, 'outer'),
    arrowAt(inner, 0.23, 'inner'), arrowAt(inner, 0.48, 'inner'), arrowAt(inner, 0.72, 'inner'),
  ],
  sceneryClearance: 1.5,
  overlayMaxEdge: 0.5,
  reactionDuration: 2.85,
  destination: { id: 'windmill', name: 'Windmill Garden', label: 'WINDMILL GARDEN', parcel: 'Flower seeds for the gardener', color: 0x9bd6b3 },
} as const;
