...
function samplePath(nodes: Vector3[]): Vector3[] {
  ...
  for (let i = 1; i < nodes.length; i++) {
    ...
    for (let j = 1; j <= steps; j++) {
      result.push(j === steps ? b.clone() : a.clone().multiplyScalar(Math.cos(angle * j / steps))
        .addScaledVector(direction, Math.sin(angle * j / steps)).normalize());
    }
  }
  return result;
}
...
function pathDistance(normal: Vector3, path: readonly Vector3[]): number {
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    const na = normal.dot(a), nb = normal.dot(b);
...
export class TourLayout {
  ...
  constructor() {
    ...
    const stop = <Id extends TourStopId, Level extends BayLevel | StationLevel | GardenLevel>(id: Id, level: Level): Stop<Id, Level> => {
      const pad = level.definition.pad.center;
      const normal = level.destination.normal.clone();
      return {
        id, level, destination: { ...level.destination, normal: normal.clone() },
    ...
    this.connectors = this.stops.map((from, index) => {
      ...
      return { from: index, to: toIndex, width: 1.8, path: samplePath([...outgoing, ...exterior.slice(1), ...incoming.slice(1)]) };
    });
    ...
    this.navigationPaths = this.stops.map((_, index) => ({
      wide: this.routeForLeg(index, 'wide'), short: this.routeForLeg(index, 'short'),
    }));
  }
  ...
  isInFootprint(normal: Vector3): boolean { return this.stops.some(stop => stop.level.isInFootprint(normal)); }

  isOnConnector(normal: Vector3, margin = 0): boolean {
    return this.connectors.some(connector => pathDistance(normal, connector.path) <= connector.width / 2 + margin + 1e-7);
  }
  ...
  isInSceneryClearance(normal: Vector3, margin = 1.5): boolean {
    return this.stops.some(stop => stop.level.isInSceneryClearance(normal, margin)) || this.isOnConnector(normal, margin);
  }
  ...
  routeForLeg(index: number, variant: 'wide' | 'short'): Vector3[] {
    ...
    const local = level instanceof BayLevel
      ? variant === 'wide' ? level.safeRoute : level.jumpRoute
  ...
  navigation(index: number, normal: Vector3, previousRoute: TourRouteCache | null, grounded: boolean, context: Pick<NavigationContext, 'forward'> = {}): TourNavigation {
    if (!Number.isInteger(index) || index < 0 || index > this.stops.length) throw new RangeError('Invalid Tour navigation index');
    const stop = this.stops[index] ?? this.stops[0];
    const previous = previousRoute?.index === index &&
      (!previousRoute.normal || surfaceDistance(normal, previousRoute.normal) < 3) ? previousRoute : null;
    ...
    if (index > 0 && index < this.stops.length && previous?.phase) {
      const p = stop.level.toLocal(normal), polygon = stop.level.definition.footprintPolygon;
    ...
    const cache: TourRouteCache = { index, route: local ? previous?.route ?? null : null,
      bayRoute: local ? previous?.bayRoute ?? null : null, phase, normal: normal.clone(), cursor: null, localCursor: null };
    if (!grounded) return { target: stop.destination.normal.clone(), route: cache, phase };
    let target: Vector3 | undefined;
    let route = cache.route, bayRoute = cache.bayRoute;
    ...
    if (local) {
      ...
      if (stop.level instanceof RoadLevel) {
        ...
        target = navigation.target; route = navigation.route; localCursor = navigation.cursor;
      } else {
        const navigation = stop.level.navigation(normal, bayRoute, localContext);
        target = navigation.target; bayRoute = navigation.route; localCursor = navigation.cursor;
      }
    }
    if (index === 0) return { target: target!, route: { ...cache, route, bayRoute, cursor: localCursor, localCursor }, phase };
    const path = index === this.stops.length ? this.connectors[2].path
  ...
  createEnvironment(colliders: Collider[]): BayEnvironment {
    return {
      spawnPose: clonePose(this.spawnPose), recoveryPose: clonePose(this.stops[0].entryPose),
