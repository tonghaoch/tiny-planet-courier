...
export function createTourConnectorGeometry(connector: TourConnector): BufferGeometry {
  ...
  const offset = (normal: Vector3, side: Vector3, angle: number) =>
    normal.clone().multiplyScalar(Math.cos(angle)).addScaledVector(side, Math.sin(angle)).normalize();
    ...
    for (let row = 0; row < rows; row++)
      for (let column = 0; column < columns; column++) {
        const p = point(row, column),
          q = point(row, column + 1);
        const r = point(row + 1, column),
          s = point(row + 1, column + 1);
        triangle(p, q, r);
        triangle(q, s, r);
      }
  ...
  const sector = (center: Vector3, side: Vector3, angle: number) => {
    ...
    const point = (ring: number, slice: number) =>
      offset(center, side.clone().applyAxisAngle(center, (angle * slice) / slices), (halfAngle * ring) / rings);
    for (let ring = 0; ring < rings; ring++)
      for (let slice = 0; slice < slices; slice++) {
        const p = point(ring, slice),
          q = point(ring + 1, slice);
        const r = point(ring + 1, slice + 1),
          s = point(ring, slice + 1);
        triangle(p, q, r);
