...
describe('flowing Garden routes', () => {
  it('anchors the pad at the original garden and round-trips the local frame', () => {
    ...
    for (let x = -14; x < 4; x += 1.3) for (let y = -6; y < 5; y += 1.7) {
...
describe('Garden scenery and gardener handoff', () => {
  ...
  beforeAll(() => { world = new PlanetWorld('garden'); original = new PlanetWorld(); });

  it('builds one garden and keeps its roads and parking approach clear of colliders', () => {
    expect(world.bayLevel).toBeNull(); expect(world.stationLevel).toBeNull();
    ...
    expect(original.gardenLevel).toBeNull(); expect(original.destinations).toHaveLength(3);
    for (const road of Object.values(GARDEN_LEVEL.roads)) for (const p of road.centerline) {
      const n = normal(p.x, p.y);
      for (const collider of world.colliders) expect(surfaceDistance(n, collider.normal), JSON.stringify(p)).toBeGreaterThan(collider.radius + road.width / 2);
    }
  ...
  it('renders the sampled spherical surface and visible flower-bed obstacles', () => {
    ...
    const checks = [normal(-12), normal(-12, 2), normal(0), ...[level.outerRoute, level.innerRoute].flatMap(route => [route[Math.floor(route.length * 0.35)], route[Math.floor(route.length * 0.65)]])];
  ...
  it('pauses and resets the handoff and blooms without allocating or mutating pooled materials', () => {
    ...
    original.root.traverse(object => {
      if (object instanceof Mesh && object.material instanceof MeshStandardMaterial) originalMaterials.set(object.material, { color: object.material.color.getHex(), glow: object.material.emissiveIntensity });
    });
    for (let attempt = 0; attempt < 3; attempt++) {
      world.resetDelivery();
      const initial = world.getDeliveryReactionSnapshot();
      expect(initial.bloom).toBe(0); expect(initial.flowersBloomed).toBe(false);
      ...
      const paused = world.getDeliveryReactionSnapshot(), scale = flowers.children[0].scale.clone(), rotation = rotor.rotation.z;
      ...
      expect(finished.bloom).toBe(1); expect(finished.flowersBloomed).toBe(true);
      ...
      for (const [mat, before] of originalMaterials) expect({ color: mat.color.getHex(), glow: mat.emissiveIntensity }).toEqual(before);
      world.resetDelivery();
  ...
  it('keeps the same blooming outcome with restrained reduced-motion feedback', () => {
    ...
    expect(arm.rotation.z).toBeCloseTo(-0.3);
    quiet.update(3, 4.7);
    expect(quiet.getDeliveryReactionSnapshot()).toMatchObject({ progress: 1, bloom: 1, flowersBloomed: true, parcelVisible: true });
  });
});
