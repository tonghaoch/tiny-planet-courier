...
export interface AuthoredLevelDefinition {
  ...
  readonly ramp?: {
    readonly base: LevelPoint;
    readonly lip: LevelPoint;
    readonly length: number;
    readonly width: number;
    readonly rise: number;
    readonly lipPitch: number;
  };
...
export function road(centerline: readonly LevelPoint[], width: number) {
  const sides = [-1, 1].map(side =>
    centerline.map((p, i) => {
      ...
      let nx = -(p.y - a.y) / beforeLength,
        ny = (p.x - a.x) / beforeLength;
      let mx = -(b.y - p.y) / afterLength,
        my = (b.x - p.x) / afterLength;
      if (i === 0) {
        nx = mx;
        ny = my;
      }
      ...
      return point(
        p.x + (side * width * (nx + mx)) / (2 * denominator),
        p.y + (side * width * (ny + my)) / (2 * denominator),
      );
    }),
  );
...
export function densify(points: readonly LevelPoint[], spacing = 0.22): LevelPoint[] {
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    for (let step = 1; step <= count; step++)
      result.push(point(a.x + ((b.x - a.x) * step) / count, a.y + ((b.y - a.y) * step) / count));
  }
  return result;
}
...
export class AuthoredLevel {
  ...
  constructor(readonly definition: AuthoredLevelDefinition) {
    ...
    this.east = new Vector3(
      Math.cos((anchor.longitude * Math.PI) / 180),
      0,
      -Math.sin((anchor.longitude * Math.PI) / 180),
    );
  ...
  toNormal(x: number, y: number): Vector3 {
    ...
    return this.anchor
      .clone()
      .multiplyScalar(Math.cos(distance / radius))
      .addScaledVector(this.east, x * scale)
      .addScaledVector(this.north, y * scale)
      .normalize();
  }

  toLocal(normal: Vector3): LevelPoint {
    const n = normal.clone().normalize();
    const east = n.dot(this.east),
      north = n.dot(this.north);
  ...
  private pose(p: LevelPoint, heading: LevelPoint): SurfacePose {
    ...
    const forward = this.toNormal(p.x + heading.x * 0.001, p.y + heading.y * 0.001).sub(
      this.toNormal(p.x - heading.x * 0.001, p.y - heading.y * 0.001),
    );
  ...
  isInFootprint(n: Vector3): boolean {
    return inLevelPolygon(this.toLocal(n), this.definition.footprintPolygon);
  }
  ...
  sampleSurface(n: Vector3): BaySurface {
    ...
    if (
      ramp &&
      p.x >= ramp.base.x - EPSILON &&
      p.x <= ramp.lip.x + EPSILON &&
      Math.abs(p.y - ramp.base.y) <= ramp.width / 2 + EPSILON
    ) {
      const rampProgress = Math.max(0, Math.min(1, (p.x - ramp.base.x) / ramp.length));
      return {
        kind: 'ramp',
        radius: this.rampRadiusAt(p.x),
        rampProgress,
    ...
    if (
      Object.values(roads).some(road => inLevelPolygon(p, road.polygon)) ||
      Math.hypot(p.x - pad.center.x, p.y - pad.center.y) <= pad.radius + EPSILON
    ) {
  ...
  crossRampLip(previous: Vector3, next: Vector3): RampCrossing | null {
    ...
    if (!ramp) return null;
    const a = this.toLocal(previous),
      b = this.toLocal(next);
    const dx = b.x - a.x;
    // Arrive from ON the eastward ramp, not its back or side, or in reverse.
    if (
      dx <= EPSILON ||
      a.x < ramp.base.x - EPSILON ||
      a.x >= ramp.lip.x - EPSILON ||
      b.x < ramp.lip.x - EPSILON ||
      Math.abs(a.y - ramp.base.y) > ramp.width / 2 + EPSILON
    )
      return null;
