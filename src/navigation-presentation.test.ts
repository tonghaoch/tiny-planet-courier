...
describe('displayed-angle animation', () => {
  it('retargets toward -20 from the actually displayed angle, not the old 170 endpoint', () => {
    let displayed = advanceNavigationHeading(0, radians(170), .06);
  ...
  it('is frame-rate independent and settles large turns in 150ms', () => {
    ...
    expect(radians(170) - advanceNavigationHeading(0, radians(170), .15)).toBeLessThan(radians(9));
  });
  it('freezes with zero dt and snaps on reset or reduced motion', () => {
    ...
    expect(advanceNavigationHeading(1, -2, .016, true)).toBe(-2);
  });
});
...
describe('arrival presentation does not widen delivery eligibility', () => {
  it('enters only inside the actual radius, exits with a quarter-unit tolerance, and requires ground', () => {
    ...
    expect(arrivalActive(false, DELIVERY_RADIUS - .001, true)).toBe(true);
    expect(arrivalActive(true, DELIVERY_RADIUS + .24, true)).toBe(true);
    expect(arrivalActive(true, DELIVERY_RADIUS + .25, true)).toBe(false);
  ...
  it('keeps parking across the center and never makes the tolerance, speed or airborne state eligible', () => {
    ...
    run.start();
    let arrival = false;
    for (const offset of [-.02, 0, .02]) arrival = arrivalActive(arrival, Math.abs(offset), true);
    expect(arrival).toBe(true);
    const outside = normal.clone().applyAxisAngle(new Vector3(1, 0, 0), (DELIVERY_RADIUS + .1) / PLANET_RADIUS);
    ...
    expect(run.update(DELIVERY_HOLD + 1, normal, 0, .2)).toBeNull();
    expect(run.parkedFor).toBe(0);
    expect(run.update(DELIVERY_HOLD - .01, normal, 0)).toBeNull();
    expect(run.update(.02, normal, 0)?.finished).toBe(true);
  });
});
