...
describe('authored Bay Leap surface', () => {
  it('round-trips geodesic local metres and the anchored east/north frame', () => {
    ...
    for (let x = -11; x <= 12; x += 1.3)
      for (let y = -6; y <= 9; y += 1.7) {
        const n = normal(x, y),
  ...
  it('owns one closed water polygon, with an open southern mouth and no hidden bridge', () => {
    ...
    for (let i = 0; i < BAY_LEVEL.waterPolygon.length; i++) {
      const a = BAY_LEVEL.waterPolygon[i],
        b = BAY_LEVEL.waterPolygon[(i + 1) % BAY_LEVEL.waterPolygon.length];
      const x = (a.x + b.x) / 2,
        y = (a.y + b.y) / 2;
      const length = Math.hypot(b.x - a.x, b.y - a.y);
      const dx = (-(b.y - a.y) / length) * 0.025,
        dy = ((b.x - a.x) / length) * 0.025;
    ...
    expect(surface(-2.31).kind).toBe('ground');
    expect(surface(2.31).kind).toBe('ground');
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      for (let step = 1; step < 10; step++)
        for (const side of [-0.85, 0, 0.85]) {
          const p = {
            x: a.x + ((b.x - a.x) * step) / 10 - ((b.y - a.y) * side) / length,
            y: a.y + ((b.y - a.y) * step) / 10 + ((b.x - a.x) * side) / length,
          };
          expect(inBayPolygon(p, BAY_LEVEL.roads.safe.polygon)).toBe(true);
          expect(surface(p.x, p.y).kind).toBe('road');
        }
    }
    for (const p of [BAY_LEVEL.spawn.position, BAY_LEVEL.recovery.position, BAY_LEVEL.pad.center])
      expect(surface(p.x, p.y).kind).toBe('road');
    const landing = BAY_LEVEL.landingRegion;
    for (const x of [landing.minX, (landing.minX + landing.maxX) / 2, landing.maxX])
      for (const y of [landing.minY, 0, landing.maxY]) expect(surface(x, y).kind).not.toBe('water');
  });
  ...
  it('shares the quadratic ramp height, slope and aligned lip crossing', () => {
    ...
    for (const t of [0, 0.25, 0.5, 0.75, 1])
      for (const y of [-0.9, 0, 0.9]) {
        const x = ramp.base.x + ramp.length * t,
          sample = surface(x, y);
...
describe('opt-in bay scenery and bakery reaction (without a browser canvas)', () => {
  ...
  it('keeps the original three destinations and baseline while opting into exactly one bakery', () => {
    expect(original.bayLevel).toBeNull();
    expect(original.bayEnvironment).toBeNull();
  ...
  it('clears both authored routes, spawn/recovery and pad of legacy colliders', () => {
    ...
    for (const n of points)
      for (const collider of world.colliders)
        expect(surfaceDistance(n, collider.normal)).toBeGreaterThan(collider.radius + 0.34);
    ...
    expect(inside[0].normal.distanceTo(normal(BAY_LEVEL.bakery.center.x, BAY_LEVEL.bakery.center.y))).toBeLessThan(
      1e-12,
    );
  });
  ...
  it('renders subdivided spherical polygons at sampled heights, without buried chords or old roads', () => {
    ...
    const surfaces = ['bay-land', 'bay-water', 'bay-road', 'bay-ramp-deck'].map(
  ...
  it('keeps all dynamic models attached after batching, oriented towards an actual recessed opening', () => {
    ...
    const toPad = world.destinations[0].normal
      .clone()
      .addScaledVector(n, -world.destinations[0].normal.dot(n))
      .normalize();
    ...
    world.root.traverse(object => {
      if (
        ...
        !isUnder(object, recipient) &&
        !isUnder(object, parcel)
      )
        solids.push(object);
    });
    ...
    world.startBayDelivery(normal(8.9).multiplyScalar(R + 0.6));
    world.update(1, 1);
    world.root.updateMatrixWorld(true);
  ...
  it('captures a WORLD parcel start, freezes only the reaction when paused, and resets every replay', () => {
    ...
    const windows = [0, 1].map(
      i => (world.root.getObjectByName(`bay-bakery-window-${i}`) as Mesh).material as MeshStandardMaterial,
    );
    ...
    original.root.traverse(object => {
      if (
        object instanceof Mesh &&
        object.material instanceof MeshStandardMaterial &&
        object.material.emissive.getHex() === 0xffc56a
      )
        legacyWindows.add(object.material);
    });
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const position = parcel.position.clone(),
        hinge = door.rotation.clone(),
        wave = arm.rotation.clone();
      const clouds = world.root.children.find(
        object =>
          object instanceof Group &&
          object.children.length === 6 &&
          object.children.every(child => child.children.length === 5),
      )!;
      ...
      expect(door.rotation.equals(hinge)).toBe(true);
      expect(arm.rotation.equals(wave)).toBe(true);
      ...
      world.update(5, 25);
      expect(world.getBayReactionSnapshot()).toEqual({
        active: false,
        progress: 1,
        doorOpen: 1,
      ...
      expect(parcel.position.length()).toBe(0);
      expect(parcel.rotation.y).toBe(0);
    }
  ...
  it('reuses a bounded splash pool and leaves legacy celebration unchanged', () => {
    ...
    let visible = 0;
    world.root.traverse(object => {
      if (object.name === 'bay-splash-particle' && object.visible) visible++;
    });
    expect(visible).toBe(0);
    world.splash(normal(0));
    expect(meshCount(world.root)).toBe(before + 20);
    const legacyCount = meshCount(original.root);
    original.splash(normal(0));
    original.startBayDelivery(normal(0));
    original.resetBayDelivery();
