...
export function unwrapNavigationHeading(previous: number | null, canonical: number): number {
  return previous === null ? canonical : previous + Math.atan2(Math.sin(canonical - previous), Math.cos(canonical - previous));
}
...
export function advanceNavigationHeading(displayed: number | null, canonical: number, dt: number, reducedMotion = false): number {
