...
function segments(normal: Vector3, path: readonly Vector3[], radius: number) {
  ...
  for (let i = 1; i < path.length; i++) {
    const a = path[i - 1].clone().normalize(),
      b = path[i].clone().normalize();
    const arc = angle(a, b);
    ...
    projections.push({
      point,
      segment: i - 1,
      progress: lengths[i - 1] + along * radius,
      distance: angle(n, point) * radius,
    });
...
export function routeAhead(
  ...
  if (cursor?.path === path && Number.isFinite(cursor.progress)) {
    ...
    if (movement < 3 && projection.distance < 1.8) {
      ...
      const tracked = nearby.reduce<RouteProjection | null>(
        (best, p) => (!best || p.distance < best.distance ? p : best),
        null,
      );
  ...
  if (i < path.length - 1) {
    const a = path[i].clone().normalize(),
      b = path[i + 1].clone().normalize();
  ...
  return {
    ...
    targetProgress,
    length,
    cursor: { path, segment: projection.segment, progress: projection.progress, normal: data.normal },
  };
}
...
export function branchNavigation<Route extends string>(
  ...
  paths: Readonly<Record<Route, readonly Vector3[]>>,
  choices: readonly [Route, Route],
  fork: Vector3,
  spawn: Vector3,
  destination: Vector3,
  ...
  const scores = projections.map(
    p => p.distance + (forward ? forkWeight * 0.85 * (1 - forward.dot(tangent(p.target, normal))) : 0),
  );
  let route = previous;
