...
describe('authored navigation discontinuity regressions', () => {
  it('removes the exact Tour Bay y=1.3 jump with and without forward/cursor context', () => {
    ...
    for (const tracking of [false, true]) {
      ...
      for (const y of [1.299, 1.301, 1.299, 1.301]) {
        ...
        const direct = bay.navigation(
          normal,
          tracking ? (cache?.bayRoute ?? null) : null,
          tracking ? { forward, cursor: cache?.localCursor } : {},
        );
  ...
  it('removes the exact reachable Garden sample jump and looks 1.8 arc units ahead', () => {
    ...
    for (const tracking of [false, true]) {
      ...
      const headings: number[] = [];
      for (const x of [-7.54, -7.536, -7.54, -7.536]) {
        const normal = level.toNormal(x, 1),
          forward = forwardAt(level, x, 1);
        expect(level.sampleSurface(normal).kind).toBe('road');
        for (const obstacle of [...GARDEN_LEVEL.beds, GARDEN_LEVEL.welcomeBed]) {
          expect(surfaceDistance(normal, level.toNormal(obstacle.center.x, obstacle.center.y))).toBeGreaterThan(
            obstacle.radius + 0.34,
          );
        }
  ...
  it('holds Bay shared approach/fork choices under noise and respects deliberate intent', () => {
    ...
    let previous: BayRoute | null = null,
      cursor: RouteCursor | null = null;
    ...
    previous = north.route;
    cursor = north.cursor;
    for (let i = 0; i < 40; i++) {
      const x = -6 + 0.004 * Math.sin(i),
        y = 0.004 * Math.cos(i);
      const normal = bay.toNormal(x, y);
      const result = bay.navigation(normal, previous, { cursor, forward: forwardAt(bay, x, y, 0.71, 0.7) });
  ...
  it.each([
    ['Station', new StationLevel(), STATION_LEVEL],
    ['Garden', new GardenLevel(), GARDEN_LEVEL],
  ] as const)(
    'keeps %s branch commitment under tiny road noise and permits switching/reset',
    (_, level, definition) => {
      for (const route of ['inner', 'outer'] as const) {
        ...
        for (let i = 15; i < path.length - 15; i += 3) {
          const normal = path[i],
            forward = tangent(path[i + 1], normal);
          const side = new Vector3().crossVectors(forward, normal).normalize();
          const a = normal
            .clone()
            .addScaledVector(side, -0.001 / 15)
            .normalize();
          const b = normal
            .clone()
            .addScaledVector(side, 0.001 / 15)
          ...
          expect(
            degrees(headingTo(a, tangent(forward, a), first.target), headingTo(b, tangent(forward, b), second.target)),
          ).toBeLessThan(0.5);
          previous = second.route;
  ...
  it('reverses authored progress and reacquires after off-road travel or branch/level changes', () => {
    ...
    for (let y = 2.4; y >= 0.8; y -= 0.1) {
      const next = bay.navigation(bay.toNormal(-6, y), result.route, {
        cursor: result.cursor,
        forward: forwardAt(bay, -6, y, 0, 1),
      });
...
describe('Tour leg/region state and pure read contract', () => {
  it('keeps targets continuous across transfer/local boundaries in both directions', () => {
    ...
    for (const x of Array.from({ length: 121 }, (_, i) => -13.6 + i * 0.01)) {
      const normal = level.toNormal(x, 0),
        result = layout.navigation(1, normal, cache, true);
      phases.add(result.phase);
      if (previous) expect(surfaceDistance(result.target, previous.target)).toBeLessThan(0.012);
      previous = { normal, target: result.target };
      cache = result.route;
    }
  ...
  it('invalidates caches across legs, teleports, recovery, flight, and closing free roam', () => {
    const layout = new TourLayout(),
      station = layout.stops[1].level,
      garden = layout.stops[2].level;
    ...
    for (const index of [-1, 0.5, 4, NaN])
      expect(() => layout.navigation(index, normal, null, true)).toThrow(RangeError);
  });

  it('is passive and does not mutate pose, cache, authored paths or destinations', () => {
    const layout = new TourLayout(),
      bay = layout.stops[0].level;
    const normal = bay.toNormal(-6, 1.3),
      forward = forwardAt(bay, -6, 1.3, 0, 1);
    ...
    a.target.set(0, 0, 0);
    a.route.normal!.set(0, 0, 0);
    a.route.cursor!.normal.set(0, 0, 0);
    expect(JSON.stringify(inputs)).toBe(before);
